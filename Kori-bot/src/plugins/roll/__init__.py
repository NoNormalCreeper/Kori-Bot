...
async def handle_roll(bot: Bot, event: GroupMessageEvent, state: T_State=State()):
    ...
    if re.match(r'^\d+[d]\d+$', _roll):
        ...
        await roll.finish('格式不对呢, 请重新输入: /rd <x>d<y>:')

    # 加入一个趣味的机制
    if random.randint(1, 100) == 99:
        await roll.finish('【彩蛋】骰子之神似乎不看好你, 你掷出的骰子全部消失了😥')
    if dice_num > 1024 or dice_side > 1024:
        await roll.finish('【错误】谁没事干扔那么多骰子啊😅')
    if dice_num <= 0 or dice_side <= 0:
        await roll.finish('【错误】你掷出了不存在的骰子, 只有上帝知道结果是多少🤔')
    dice_result = 0
