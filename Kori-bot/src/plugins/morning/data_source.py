...
MORNING_PATH = _MORNING_PATH or os.path.join(
...
class MorningManager:
    ...
    def get_current_config(self) -> str:
        msg = '早安晚安设置如下：'
        if get_up_intime := self.config['morning']['get_up_intime']['enable']:
        ...
        if sleep_intime := self.config['night']['sleep_intime']['enable']:
...
# ------------------------------ config ------------------------------ #
    ...
    def change_set_time(self, *args) -> str:
        try:
            day_or_night = args[0]
            server = args[1]
            if server in ["get_up_intime", "sleep_intime"]:
                early_time = args[2]
                late_time = args[3]
...
# ------------------------------ morning judgement------------------------------ #
    ...
    def judge_mor_time(early_time_tmp: str, late_time_tmp: str, now_time: datetime.datetime) -> bool:
        early_time = datetime.datetime.strptime(
            f'{str(datetime.datetime.now().date())} {early_time_tmp}:00:00',
            '%Y-%m-%d %H:%M:%S',
        )

        late_time = datetime.datetime.strptime(
            f'{str(datetime.datetime.now().date())} {late_time_tmp}:00:00',
            '%Y-%m-%d %H:%M:%S',
        )

        return now_time >= early_time and now_time <= late_time
    ...
    def judge_have_mor(self, group_id: str, user_id: str, now_time: datetime.datetime, interval: str) -> bool:
        ...
        return get_up_time + datetime.timedelta(hours = int(interval)) > now_time
    ...
    def judge_super_get_up(self, group_id: str, user_id: str, now_time: datetime.datetime, interval: str) -> bool:
        ...
        return sleep_time + datetime.timedelta(hours = int(interval)) > now_time
    ...
    def morning_and_update(self, now_time: datetime.datetime, group_id: str, user_id: str) -> Tuple[str, Union[int, str]]:
        ...
        in_sleep_tmp = f'{int(hour)}时{int(minute)}分{int(second)}秒' if day == 0 else 0
...
# ------------------------------ night judgement ------------------------------ #
    ...
    def judge_sle_time(early_time_tmp: str, late_time_tmp: str, now_time: datetime.datetime) -> bool:
        ...
        return now_time >= early_time or now_time <= late_time
    ...
    def night_and_update(self, now_time: datetime.datetime, group_id: str, user_id: str) -> Tuple[str, Union[int, str]]:
        ...
        if self.user_data[group_id][user_id]['get_up_time'] != 0:
            ...
            second = secs - day * 3600 * 24 - hour * 3600 - minute * 60
            if day == 0:
                in_day_tmp = f'{int(hour)}时{int(minute)}分{int(second)}秒'
        # 判断是今天第几个睡觉的
...
# ------------------------------ routine ------------------------------ #
    ...
    def get_group_routine(self, event: GroupMessageEvent) -> str:
        ...
        return f'今天已经有{moring_count}位群友起床了，{night_count}群友睡觉了~'

morning_manager = MorningManager(Path(MORNING_PATH))