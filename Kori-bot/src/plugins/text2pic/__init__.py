...
# 纯文本转图片
text2pic = on_command("text2pic",aliases={'t2p','ttp'})
...
# 加载本地 html 方法
html2pic = on_command("html2pic", aliases={'h2p','htp'})


@html2pic.handle()
async def _html2pic(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    from pathlib import Path
...
# 使用 md2pic
md2pic = on_command("md2pic", aliases={'m2p','mtp'})


@md2pic.handle()
async def _md2pic(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    from pathlib import Path
